...
public interface OrderRepository extends JpaRepository<Order, Long> {
        ...
        Page<Order> findByStatusAndIsDeletedFalse(com.example.orderservice.enums.EnumProcessOrder status,
                        Pageable pageable);

        // 🔍 Lấy orders theo storeId và status (optional), sắp xếp theo thời gian tạo
        // (mới nhất trước)
        @Query("""
        ...
        // 🔍 Lấy tất cả orders của store (không filter status), sắp xếp theo thời gian
        // tạo (mới nhất trước)
        @Query("""
                            SELECT o FROM Order o
        ...
        // Orders có pdfFilePath IS NOT NULL AND pdfFilePath != '' được coi là đã tạo
        // hóa đơn
        @Query("""
                            SELECT o FROM Order o
        ...
        Double getTotalRevenueByStatuses(
