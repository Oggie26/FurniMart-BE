...
public interface WarrantyRepository extends JpaRepository<Warranty, Long> {
    ...
    List<Warranty> findActiveWarrantiesByCustomer(@Param("customerId") String customerId, @Param("now") LocalDateTime now);
    
    @Query("SELECT w FROM Warranty w WHERE w.orderId = :orderId AND w.orderDetailId = :orderDetailId AND w.isDeleted = false")
    Optional<Warranty> findByOrderIdAndOrderDetailId(@Param("orderId") Long orderId, @Param("orderDetailId") Long orderDetailId);
    ...
    List<Warranty> findActiveWarrantiesByProductAndCustomer(@Param("productColorId") String productColorId, @Param("customerId") String customerId);
