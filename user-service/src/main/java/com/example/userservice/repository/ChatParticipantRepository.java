...
public interface ChatParticipantRepository extends JpaRepository<ChatParticipant, String> {
    ...
    @Query("SELECT p FROM ChatParticipant p WHERE (p.user.id = :userId OR p.employee.id = :userId) AND p.status = 'ACTIVE'")
    List<ChatParticipant> findActiveParticipationsByUserId(@Param("userId") String userId);

    @Query("SELECT p FROM ChatParticipant p WHERE p.chat.id = :chatId AND (p.user.id = :userId OR p.employee.id = :userId) AND p.status = 'ACTIVE'")
    Optional<ChatParticipant> findActiveParticipantByChatIdAndUserId(@Param("chatId") String chatId, @Param("userId") String userId);

    @Query("SELECT p FROM ChatParticipant p WHERE p.chat.id = :chatId AND p.employee.id = :employeeId AND p.status = 'ACTIVE'")
    Optional<ChatParticipant> findActiveParticipantByChatIdAndEmployeeId(@Param("chatId") String chatId, @Param("employeeId") String employeeId);

    @Query("SELECT p FROM ChatParticipant p WHERE p.employee.id = :employeeId AND p.status = 'ACTIVE'")
    List<ChatParticipant> findActiveParticipationsByEmployeeId(@Param("employeeId") String employeeId);
    ...
    @Query("SELECT p FROM ChatParticipant p WHERE p.chat.id = :chatId AND (p.user.id = :userId OR p.employee.id = :userId)")
    Optional<ChatParticipant> findByChatIdAndUserId(@Param("chatId") String chatId, @Param("userId") String userId);

    @Query("SELECT p FROM ChatParticipant p WHERE (p.user.id = :userId OR p.employee.id = :userId) AND p.chat.type = :chatType AND p.status = 'ACTIVE'")
