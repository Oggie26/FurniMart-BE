...
public class DeliveryController {
        ...
        @Operation(summary = "Get store branch information with stock availability", description = "Retrieve detailed store information including warehouse details and stock availability. This is a public API that does not require authentication.")
        @ApiResponses(value = {
        ...
        @PostMapping("/assign")
        @Operation(summary = "Assign order to delivery staff", description = "Assign an order to a delivery staff member. Only STAFF and BRANCH_MANAGER roles can use this API. "
                        +
        ...
        public ApiResponse<DeliveryAssignmentResponse> assignOrderToDelivery(
        ...
        // description = "Generate an invoice (PDF) for an order. Only STAFF and
        // BRANCH_MANAGER roles can use this API. " +
        // "Order status must be READY_FOR_INVOICE (or MANAGER_ACCEPT for backward
        // compatibility). " +
        // "Each order can only have an invoice generated once. " +
        // "After successful generation, invoiceGenerated will be set to true and PDF
        ...
        // description = "Invoice already generated for this order OR Order status is
        // not READY_FOR_INVOICE/MANAGER_ACCEPT"),
        // @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404",
        // description = "Order not found with orderId: {orderId}"),
        // @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401",
        // description = "Unauthenticated"),
        // @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403",
        // description = "Access denied - Only STAFF and BRANCH_MANAGER roles are
        // allowed")
        // })
        // @ResponseStatus(HttpStatus.OK)
        // @PreAuthorize("hasRole('STAFF') or hasRole('BRANCH_MANAGER')")
        // public ApiResponse<DeliveryAssignmentResponse> generateInvoice(@PathVariable
        // Long orderId) {
        // return ApiResponse.<DeliveryAssignmentResponse>builder()
        // .status(HttpStatus.OK.value())
        ...
        @Operation(summary = "Prepare products for delivery", description = "Prepare products for delivery. STAFF and BRANCH_MANAGER roles can use this API. "
                        +
        ...
        @ResponseStatus(HttpStatus.OK)
        @PreAuthorize("hasRole('STAFF') or hasRole('BRANCH_MANAGER')")
        public ApiResponse<DeliveryAssignmentResponse> prepareProducts(
                        @Valid @RequestBody PrepareProductsRequest request) {
                return ApiResponse.<DeliveryAssignmentResponse>builder()
                                .status(HttpStatus.OK.value())
        ...
        @Operation(summary = "Get delivery assignments by store", description = "Retrieve all delivery assignments for a specific store. Only STAFF and BRANCH_MANAGER roles can use this API.")
        @ApiResponses(value = {
        ...
        @PreAuthorize("hasRole('STAFF') or hasRole('BRANCH_MANAGER')")
        public ApiResponse<List<DeliveryAssignmentResponse>> getDeliveryAssignmentsByStore(
        ...
        @Operation(summary = "Get delivery assignment by order ID", description = "Retrieve delivery assignment information for a specific order. Only STAFF and BRANCH_MANAGER roles can use this API.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Monitor delivery progress within branch", description = "Monitor delivery progress for a store. Only BRANCH_MANAGER role can use this API. "
                        +
        ...
        @Operation(summary = "Update delivery status", description = "Update delivery status. Only BRANCH_MANAGER and DELIVERY roles can use this API. "
                        +
        ...
        public ApiResponse<DeliveryAssignmentResponse> updateDeliveryStatus(
                        @Parameter(description = "Delivery assignment ID", required = true, example = "1") @PathVariable Long assignmentId,
                        @Parameter(description = "Delivery status. Valid values: ASSIGNED, PREPARING, READY, IN_TRANSIT, DELIVERED, CANCELLED", required = true, schema = @Schema(type = "string", allowableValues = {
                                        "ASSIGNED", "PREPARING", "READY", "IN_TRANSIT", "DELIVERED",
                                        "CANCELLED" }, example = "IN_TRANSIT")) @RequestParam String status) {
                return ApiResponse.<DeliveryAssignmentResponse>builder()
                                .status(HttpStatus.OK.value())
        ...
        @Operation(summary = "Get delivery assignments by delivery staff", description = "Retrieve all delivery assignments for a specific delivery staff member. Only DELIVERY role can use this API.")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Delivery assignments retrieved successfully (may return empty list if no assignments exist)"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthenticated"),
        ...
        public ApiResponse<List<DeliveryAssignmentResponse>> getDeliveryAssignmentsByStaff(
        ...
        @Operation(summary = "Reject delivery assignment", description = "Delivery staff can reject an assignment assigned to them. "
                        +
        ...
        public ApiResponse<DeliveryAssignmentResponse> rejectAssignment(
                ...
                return ApiResponse.<DeliveryAssignmentResponse>builder()
                                ...
                                .data(deliveryService.rejectAssignment(assignmentId, request.getReason(),
                                                deliveryStaffId))
                                .build();
        }

        @GetMapping("/drivers/free")
        @Operation(summary = "Get free delivery drivers")
        @PreAuthorize("hasRole('STAFF') or hasRole('BRANCH_MANAGER')")
        public ApiResponse<List<com.example.deliveryservice.response.UserResponse>> getFreeDrivers() {
                return ApiResponse.<List<com.example.deliveryservice.response.UserResponse>>builder()
                                .status(HttpStatus.OK.value())
                                .message("Free delivery drivers retrieved successfully")
                                .data(deliveryService.getFreeDrivers())
                                .build();
        }
