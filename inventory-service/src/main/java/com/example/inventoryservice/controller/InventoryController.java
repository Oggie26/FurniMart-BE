...
public class InventoryController {
        ...
        @Operation(summary = "Kiểm tra tồn kho tại store cụ thể (cho AI recommendation)")
        @GetMapping("/stock/check-at-store")
        public ApiResponse<Boolean> checkStockAtStore(
                        @RequestParam @NotBlank String productColorId,
                        @RequestParam @NotBlank String storeId,
                        @RequestParam @Min(1) Integer quantity) {

                boolean hasStock = inventoryService.hasSufficientStock(productColorId, storeId, quantity);

                return ApiResponse.<Boolean>builder()
                                .status(200)
                                .message("Kiểm tra tồn kho tại store thành công")
                                .data(hasStock)
                                .build();
        }
        ...
        public ApiResponse<ProductLocationResponse> getProductLocationsByWarehouse(
                ...
                ProductLocationResponse response = inventoryService.getProductLocationsByWarehouse(productColorId,
                                storeId);
        ...
        public ApiResponse<InventoryWarehouseViewResponse> getWarehouseInventoryView(
                ...
                InventoryWarehouseViewResponse response = inventoryService.getWarehouseInventoryView(warehouseId);
